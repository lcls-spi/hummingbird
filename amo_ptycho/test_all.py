...
pixel_size = siemens_radius / image_side #so siemens_radius assumed to cover all FOV
...
def create_mode(rampxy):
    ...
    return np.fft.ifftn( np.roll(
                         np.roll( np.fft.fftn(illumination), rampxy[0], axis=0),                                                             rampxy[1], axis=1) )
...
#%%
# Load sample
siemens = np.array(Image.open('/Users/simone/Documents/Simone/Squola/2014-2015 (PhD)/simulations/LCLS_simul/Siemens_star.png').getdata()).reshape((image_side,image_side,4))
...
for i in range(np.shape(positions)[0]):
    ...
    dontprint = plt.axis('off')
    plt.subplot(1,2,2)
    plt.imshow(abs(sample))
    dontprint = plt.axis('off')
    plt.savefig('/Users/simone/Documents/Simone/Squola/2014-2015 (PhD)/simulations/img/Siemens_%s.png' %str(1000+i))
    plt.close()
#%%
#######################################################
#######################################################
...
for i in range(50):
    ...
    f = np.fft.fftshift(abs(np.fft.fftn(random_mode(5.)[ofs:-ofs,ofs:-ofs]*obj1a))**2)
...
for i in range(tot**2):
    plt.subplot(tot,tot,counter)
    plt.imshow(abs(m[-1-counter]))
    #plt.imshow(abs(mo1[counter]))
    plt.axis('off')
    counter += 1
#%%
